...
if (isset($_GET['search_account'])) {
	...
	if ($stmt->rowCount() > 0) {
		foreach ($stmt->fetchALL() as $row) {
			$c++;
			$data .= '<tr>';
			$data .= '<td><p class="mb-0"><label class="mb-0"><input type="checkbox" class="singleCheck" value="' . $row['id'] . '" onclick="get_checked_length()" /><span></span></label></p></td>';
			$data .= '<td style="cursor:pointer;" class="modal-trigger" data-toggle="modal" data-target="#update_account" onclick="get_accounts_details(&quot;' . $row['id'] . '~!~' . $row['id_number'] . '~!~' . $row['username'] . '~!~' . $row['full_name'] . '~!~' . $row['password'] . '~!~' . $row['section'] . '~!~' . $row['role'] . '&quot;)">' . $c . '</td>';
